...
class HeraldInferenceTester:
    ...
    def run_test_suite(self, num_examples: int, max_generation_steps: int):
        ...
        if inference_result['success']:
            ...
            # Handle case where non-main processes might have empty generated_texts
            if len(generated_texts) == 0:
                self.log("No generated texts available on this process. Skipping evaluation.", level="warning")
                # Create placeholder results for non-main processes
                for example in original_examples:
                    results.append({
                        'example': example,
                        'generated_proof': "No text generated on this process",
                        'evaluation': {'exact_match': False, 'tactic_similarity': 0.0, 'verified': False},
                        'total_batch_time': inference_result['inference_time']
                    })
            else:
                # Trim generated texts to match original examples (remove padding)
                generated_texts = generated_texts[:len(original_examples)]
                ...
                for i, (example, text) in enumerate(zip(original_examples, generated_texts)):
                    ...
                    # Construct full Lean code for verification
            ...
            # Create error results for all examples
            for example in original_examples:
                results.append({
                    'example': example, 
        ...
        # Only print summary and save results on main process
        if jax.process_index() == 0:
