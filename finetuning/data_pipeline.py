...
from datasets import load_from_disk
from finetuning.config import PRETOKENIZED_DATASET_DIR, MAX_SEQ_LEN
import jax # Import jax to get process count and index

def get_dataset(split: str, batch_size: int, shuffle: bool = True):
    """
    Loads the pre-tokenized dataset and prepares a unique shard for each host.
    """
    try:
    ...
    # Convert to a TensorFlow dataset. from_generator is a reliable way to do this.
    tf_dataset = tf.data.Dataset.from_generator(
        lambda: iter(dataset), # The generator is simply an iterator over the HF dataset
        output_signature={
            # Define the structure and types of the dataset elements
    ...
    # Shard the dataset so each host sees a unique, non-overlapping part.
    # This is the critical fix for multi-host training.
    ...
    if shuffle:
        # Now, each host shuffles only its own slice of the data.
        # A larger buffer size provides better shuffling.
        tf_dataset = tf_dataset.shuffle(buffer_size=10_000)

    # Apply padding, batch the data, and prefetch for performance.
    tf_dataset = tf_dataset.padded_batch(
        ...
        padding_values={
            "input_ids": 0,
            "attention_mask": 0,
            "labels": -100, # Pad labels with -100 to be ignored by the loss function
            "segment_pos": 0,
        },
        drop_remainder=True # Drop the last batch if it's smaller than the batch size
    )
    
    # Prefetch data to overlap data loading with model computation.
    tf_dataset = tf_dataset.prefetch(tf.data.AUTOTUNE)

    return tf_dataset

if __name__ == "__main__":
    # This block allows you to test the data pipeline directly.
    from finetuning.config import TRAIN_SPLIT, BATCH_SIZE, TOK_FILE
    import sentencepiece as spm
    import numpy as np

    # Mock JAX for single-process testing
    class MockJax:
    ...
    for batch in train_dataset.take(1):
        ...
        # Decode the first example in the batch for verification
        first_example_ids = batch["input_ids"][0].numpy()
        first_example_labels = batch["labels"][0].numpy()

        # Find where the actual content ends (where padding or masked labels begin)
        first_real_token_len = np.sum(batch["attention_mask"][0].numpy())
        
        print(f"\nSequence length (including padding): {len(first_example_ids)}")
        print(f"Actual content length: {first_real_token_len}")

        decoded_input = vocab.decode(first_example_ids.tolist()[:first_real_token_len])
        print("\nDecoded Input (first example):\n", decoded_input)

        # Decode only the parts that are meant to be predicted
        proof_labels = [token for token in first_example_labels if token != -100]
        decoded_proof = vocab.decode(proof_labels)
        print("\nDecoded Target Labels (proof part):\n", decoded_proof)
