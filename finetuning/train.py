...
from utils.model_loader import load_recurrent_gemma_model
from finetuning.data_pipeline import get_dataset
from finetuning.sharding import get_recurrent_gemma_sharding_spec
from finetuning.config import (
    CKPT_DIR, TOK_FILE, TRAIN_SPLIT, LEARNING_RATE, BATCH_SIZE,
    NUM_EPOCHS, GRADIENT_ACCUMULATION_STEPS, CHECKPOINT_DIR, WEIGHT_DTYPE,
)
...
def main():
    jax.distributed.initialize()
    if jax.process_index() == 0:
        print(f"JAX initialized on {jax.process_count()} processes and {jax.device_count()} devices.")
    ...
    lr = optax.cosine_decay_schedule(init_value=LEARNING_RATE, decay_steps=total_steps, alpha=0.1)
    opt = optax.chain(optax.clip_by_global_norm(1.0), optax.adafactor(learning_rate=lr))
    ...
    param_sharding_spec = get_recurrent_gemma_sharding_spec(state_cpu.params)
    sharding_spec = state_cpu.replace(
        step=PartitionSpec(), params=param_sharding_spec,
        opt_state=get_recurrent_gemma_sharding_spec(state_cpu.opt_state),
        accum_grads=param_sharding_spec,
    )
    state = jax.device_put(state_cpu, jtu.tree_map(lambda s: NamedSharding(mesh, s), sharding_spec))
    del state_cpu

    p_train_step = shard_map(
        _train_step_core, mesh=mesh,
        in_specs=(sharding_spec, PartitionSpec("fsdp"), None, None),
        out_specs=(sharding_spec, None), check_rep=False,
    )
    p_apply_grads = shard_map(
        apply_grads, mesh=mesh, in_specs=(sharding_spec,),
        out_specs=sharding_spec, check_rep=False
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for step, batch in enumerate(pbar):
            batch = jtu.tree_map(
                lambda x: multihost_utils.host_local_array_to_global_array(x, mesh, PartitionSpec("fsdp")),
                batch,
            )
            state, loss = p_train_step(state, batch, rng, state.step)
            tot_loss += loss
