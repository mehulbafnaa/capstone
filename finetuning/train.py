...
# ------------------------------------------------------------------
def safe_fsdp_sharding(pytree, axis_name, num_devices):
    """Shard only tensors whose axis-0 length is divisible by num_devices."""
    def spec(_, x):
        if x.ndim >= 1 and x.shape[0] >= num_devices and x.shape[0] % num_devices == 0:
            return PartitionSpec(axis_name, *([None] * (x.ndim - 1)))
        return PartitionSpec()
...
def main():
    ...
    with Mesh(jax.devices(), axis_names=("fsdp",)) as mesh:
        ...
        # --- CORRECTED MODEL LOADING AND STATE CREATION PATTERN ---

        # 1. Build the model config and instance FIRST
        if jax.process_index() == 0:
            print(f"Loading tokenizer from: {TOK_FILE}")
        vocab = spm.SentencePieceProcessor(model_file=str(TOK_FILE))
        preset = rg.Preset.RECURRENT_GEMMA_2B_V1
        ...
        # 2. Create an ABSTRACT sharded state to define the target structure for Orbax
        #    We use jax.eval_shape to create this without allocating memory.
        abstract_params = jax.eval_shape(model.init, jax.random.PRNGKey(0))['params']
        param_sharding_spec = safe_fsdp_sharding(abstract_params, "fsdp", num_devices)
        sharded_item_spec = jtu.tree_map(lambda s: NamedSharding(mesh, s), param_sharding_spec)

        # 3. Restore the checkpoint DIRECTLY into the sharded device array structure.
        #    Orbax uses the sharded_item_spec to load only the necessary slice for each device.
        if jax.process_index() == 0:
            print(f"Restoring checkpoint from {CKPT_DIR} with sharding...")
        
        restored_tree = ocp.PyTreeCheckpointer().restore(
            str(CKPT_DIR),
            item={'params': jax.device_put(abstract_params, sharded_item_spec)}
        )
        params = restored_tree.get("params", restored_tree)
        if WEIGHT_DTYPE is not None:
            params = jax.tree.map(lambda x: x.astype(WEIGHT_DTYPE), params)

        print_tensor_stats(params, "Parameter shapes (Post-Sharded Restore)")
        ...
        # 6. Create the FINAL sharded TrainState.
        #    `params` is already a sharded GlobalDeviceArray, so we build the state around it.
        #    No more `state_cpu` or separate `jax.device_put` for the whole state.
        #    We build the template on CPU first before putting it onto devices with the full sharding spec.
        state_template = TrainState.create(
            apply_fn=model.apply,
            params=params, # Use the already sharded params
            tx=opt,
            accum_grads=jtu.tree_map(jnp.zeros_like, params), # Grads will have same sharding as params
        )

        # Define the sharding for the full state object
        state_sharding_spec = state_template.replace(
            step=NamedSharding(mesh, PartitionSpec()),
            params=sharded_item_spec, # Reuse spec from above
            opt_state=safe_fsdp_sharding(state_template.opt_state, "fsdp", num_devices),
            accum_grads=sharded_item_spec,
        )

        # Put the state onto devices using the defined sharding
        state = jax.device_put(state_template, state_sharding_spec)
        ...
        p_train_step = shard_map(
            _train_step_core,
            mesh=mesh,
            in_specs=(state_sharding_spec, data_sharding_spec, None, None),
            out_specs=(state_sharding_spec, None),
            check_rep=False,
        )
        p_apply_grads = shard_map(
            apply_grads,
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            tot_loss = 0
            for step, batch in enumerate(pbar):
                # Ensure batch is sharded correctly across hosts and devices
                batch_sharded = jtu.tree_map(
                    lambda x: multihost_utils.host_local_array_to_global_array(
                        x, mesh, PartitionSpec("fsdp", *([None] * (x.ndim - 1)))
                    ),
                    batch,
                ...
                state, loss = p_train_step(state, batch_sharded, rng, state.step)
                tot_loss += loss
