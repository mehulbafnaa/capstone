...
def main():
    ...
    with Mesh(jax.devices(), axis_names=('data_axis',)) as device_mesh:
        ...
        global_batch_size = effective_batch_size * num_devices
        train_loader, num_train_examples = get_dataset(TRAIN_SPLIT, global_batch_size)
        
        # The number of steps per epoch is now the length of the DataLoader
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            # The PyTorch DataLoader is an iterator. tfds.as_numpy converts it for JAX.
            dataset_iter = tfds.as_numpy(pbar if jax.process_index() == 0 else train_loader)
