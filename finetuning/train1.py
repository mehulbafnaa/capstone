...
def get_config():
    ...
    c.train_split = "train"
    c.eval_split = "valid"
    c.max_seq_len = 1024 # Reduced for debugging
    c.dataset_fraction = 0.001
    c.weight_dtype = jnp.bfloat16
    c.data_axis = "data"
    c.model_axis = "model"
...
def create_dataset_iterator(config, split, mesh, batch_size):
    ...
    def gen():
        ...
        for ex in ds:
            if not required <= ex.keys():
                continue
            # DEBUG: Confirm generator is producing data
            if jax.process_index() == 0 and yielded % 50 == 0:
                print(f"[DEBUG] PID {os.getpid()}: Generator yielding item {yielded}...")
            text = (
                f"{ex.get('header', '')}\n"
                f"--- informal theorem ---\n{ex['informal_theorem']}\n"
                f"--- formal theorem ---\n{ex['formal_theorem']}\n"
    ...
    def jax_iter():
        for i, np_batch in enumerate(tf_ds.as_numpy_iterator()):
            # DEBUG: Confirm data is being pulled from the tf.data pipeline
            if jax.process_index() == 0 and i == 0:
                print(f"[DEBUG] PID {os.getpid()}: First batch fetched from tf.data. Shape: {np_batch.shape}")
            yield {
                "inputs": multihost_utils.host_local_array_to_global_array(
                    np_batch, mesh, PartitionSpec(config.data_axis)
                )
...
def make_sharded_forward_fn(model, mesh, data_axis):
    def _forward(tokens, positions, params):
        # DEBUG: This print will only execute ONCE during the JIT trace.
        # It will print from one of the TPU workers.
        jax.debug.print("[DEBUG] JAX TRACE: Tracing _forward function.")
...
def main(argv):
    ...
    # DEBUG: Basic setup info
    if jax.process_index() == 0:
        print(f"[DEBUG] JAX running on {jax.process_count()} processes and {jax.device_count()} total devices.")
        print(f"[DEBUG] Using config: global_batch_size={cfg.global_batch_size}, max_seq_len={cfg.max_seq_len}")
    ...
    model, params, pspecs = load_and_shard_model(cfg, mesh)
    if jax.process_index() == 0:
        print("[DEBUG] Model loaded and sharded onto devices.")
    ...
    for epoch in range(cfg.num_epochs):
        steps = cfg.eval_steps * 5
        if jax.process_index() == 0:
            print(f"\n[DEBUG] Starting Epoch {epoch+1}/{cfg.num_epochs}")
            pbar = tqdm(total=steps, desc=f"Epoch {epoch+1}/{cfg.num_epochs}")
        ...
        for step in range(1, steps + 1):
            ...
            if jax.process_index() == 0 and step == 1:
                print(f"[DEBUG] Epoch {epoch+1}, Step {step}: Batch fetched. Input shape: {batch['inputs'].shape}")
                print(f"[DEBUG] ==> Calling the JIT-compiled train_step function now. The hang, if any, will happen here during compilation. Timestamp: {time.time()}")
            ...
            # DEBUG: This line will only be reached if train_step completes
            if jax.process_index() == 0 and step == 1:
                 print(f"[DEBUG] <== train_step function call completed. Timestamp: {time.time()}")
            ...
            if step % cfg.eval_steps == 0:
                if jax.process_index() == 0:
                    print(f"\n[DEBUG] Starting evaluation at step {step}...")
                multihost_utils.sync_global_devices("eval")
                eval_losses = []
                for _ in range(50):
                    eval_batch = next(eval_it)
