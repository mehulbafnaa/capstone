#!/usr/bin/env python3
"""
Lean Proofs Model Inference & Verification Script (with Few-Shot Prompting)

This script loads examples from the Herald Proofs dataset, constructs a few-shot
prompt to guide the model, runs inference using RecurrentGemma, and then uses
the Lean 4 compiler to formally verify the correctness of the generated proof.

This script is designed to be run from the repository root, but it locates
all necessary paths relative to its own location inside the 'evals/' folder.
"""
...
class HeraldInferenceTester:
    ...
    def __init__(self):
        ...
        # All paths are now based on the calculated REPO_ROOT.
    ...
    def load_herald_examples(self, num_total_examples: int = 5):
    ...
    def create_few_shot_prompt(self, target_example: dict, few_shot_examples: list) -> str:
        """
        Creates a few-shot prompt to guide the model's output by showing it
        high-quality examples of solved proofs.
        """
        prompt_parts = [
    ...
    def run_inference(self, prompt: str, max_steps: int = 1000) -> dict:
        ...
        print("Running inference on few-shot prompt...")
    ...
    def run_test_suite(self, num_few_shot=2, num_to_test=3):
        """Run the complete test suite using a few-shot prompting strategy."""
        print("\n" + "=" * 80)
        print("Starting Herald Proofs Inference & Verification Test Suite (Few-Shot Strategy)")
        print("=" * 80)
        ...
        print(f"Using {len(few_shot_examples)} examples for the few-shot prompt context.")
    ...
    def _print_summary(self, results: list):
        ...
        print("\n" + "=" * 80)
        print("TEST SUITE SUMMARY (Few-Shot Verification)")
        print("=" * 80)
...
def main():
    ...
    try:
        ...
        tester.run_test_suite(num_few_shot=2, num_to_test=3)
